...
export function confirmBulkDelete(n: number): Promise<boolean> {
  ...
  return confirm({
    ...
    message:
      pushMode !== "off"
        ? "Операции скроются из всех расчётов и списков. Так как включён Push, при следующей отправке они будут удалены и в облаке Дзен-мани — это необратимо в облаке."

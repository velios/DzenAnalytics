...
describe("pushNote — что будет с записанным при текущей отправке", () => {
  it("без подключения к Дзен-мани так и говорим, а не «отправка выключена»", () => {
    expect(pushNote("auto", false).text).toBe("Останется на устройстве: Дзен-мани не подключён.");
  });

...
describe("buildPdf", () => {
  ...
  it("ориентация задаётся каждому листу отдельно", () => {
    // В отчёте бюджета сводка идёт альбомной, а списки статей — книжными
    // (issue #68): один размер на весь документ тут не подходит.
    const text = asText(
      buildPdf([page(), { ...page(), sheet: A4_PORTRAIT }], { page: A4_LANDSCAPE })
    );
    expect(text).toContain(`/MediaBox [0 0 842 595]`);
    expect(text).toContain(`/MediaBox [0 0 595 842]`);
    // И картинка растягивается на СВОЙ лист, а не на общий.
    expect(text).toContain(`595 0 0 842 0 0 cm`);
  });

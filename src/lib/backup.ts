...
export const BACKUP_EXCLUDED_KEYS: Record<string, string> = {
  ...
  // Признак «пользователь сам отключился от провайдера». Он блокирует тихое
  // авто-подключение на старте, чтобы живая сессионная кука не подхватила
  // аккаунт заново. Восстанавливать его нельзя в обе стороны: перенесённое
  // «отключён» выкинуло бы на экран выбора там, где человек залогинен, а
  // перенесённое «подключён» вернуло бы то самое тихое подключение, от
  // которого он на этом устройстве и отказался. Состояние сессии, а не
  // переносимая настройка.
  zenmoneyProviderOptOut: "отказ от авто-подключения — состояние сессии устройства",

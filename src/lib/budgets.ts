...
export interface BudgetLine {
  id: string;
  /**
   * Тег Дзен-мани, к которому привязан план. Есть у строк, пришедших из
   * синхронизации, и это НАСТОЯЩЕЕ их тождество: имена меняются (категорию
   * переименовали), а тег остаётся.
   *
   * Пока его не было, переименование заводило вторую строку рядом со старой —
   * на экране появлялись две одинаковых с виду статьи, а план родителя
   * складывался из обеих. Строки, заведённые вручную или из CSV, тега не
   * знают: их тождество — по-прежнему имена.
   */
  tagId?: string;
  category: string;
  /**

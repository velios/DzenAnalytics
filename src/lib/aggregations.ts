...
export function detectDuplicates(
  ...
  for (const t of txs) {
    ...
    // Сумма — в копейках. Округление до целых рублей склеивало всё, что меньше
...
export function topPayees(
   ...
   *
   * `payee` — это то, что напечатал банк: «DOSTAVKA YOLOCHKA» и «DOSTAVKA IZ
   * YOLOCHKI» две разные строки и одна «Ёлочка». В топе они делят сумму

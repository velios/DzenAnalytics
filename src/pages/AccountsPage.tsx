...
function DropdownMenu({
  ...
  icon: LucideIcon;
  active: boolean;
  /** Число активных фильтров — показывается кружком у подписи. */
  badge?: number;
...
/** Фильтр пройден? Пустое множество = «все», {FILTER_NONE} = «ничего». */
...
/**
 * Почему на «Капитале» половина фильтров не работает.
 *
...
export function AccountsPage() {
  ...
  // Набора «операции под фильтром» для остатков больше нет: остаток на дату
  // складывается из всей истории до неё, а фильтр выбирает лишь окно показа.
  ...
  // ВСЕМ счетам, а не по тем, что уцелели после фильтров списка.
  ...
  const accountRowsResult = useMemo(() => {
    ...
    // было движения. На «Движении» наоборот — там показаны обороты за фильтр.
    ...
    // в нём есть ВСЕГДА, с пометкой «Вне баланса» и отдельным фильтром. Раньше
  ...
  // Фильтры предлагают только те значения, что реально есть у пользователя:
  ...
  const typeOptions = useMemo(() => {
    ...
    // такого вида сейчас нет: иначе непонятно, есть ли фильтр вообще, и список
   ...
   *
   * KPI наверху считает совокупный баланс по всем счетам и фильтры не знает,
   ...
   * вовсе, плюс отсеянные фильтрами. Без этой цифры итог по списку и KPI
   ...
   * без движения больше, чем скрыто фильтрами, выходило «9 из 5». Настоящий
   * знаменатель — все известные счета: показанные, отсеянные фильтрами и спящие.
   */
   ...
   *
   * Остаток на дату складывается из ВСЕЙ истории до неё — фильтр не может его
   ...
   * двигать границы: фильтр «сумма от 10 000» в «Дополнительно» менял, какие
  ...
  const viewWindow = useMemo(() => {
    ...
    // «custom» presetToRange не разворачивает — свои даты лежат прямо в фильтре.
    const r =
  ...
  const chartAccountOptions = useMemo(() => {
    // Порядок — как в глобальном фильтре: по виду счёта, архивные внизу. Иначе
  ...
  /** Вид счёта для заголовков групп — как в глобальном фильтре. */
  ...
  /**
   * Счета, выбранные для стопки. `null` — фильтра нет, работает автоматика
   ...
   * на «Капитале» — по всей истории, на «Движении» — за фильтр.
   */
  ...
  // Пик считаем по тому, что в окне, а не «не ниже нуля»: при фильтре, где всё
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Панель больше не гаснет: фильтр в деле на обеих вкладках. На «Капитале»
          ...
          на «Движении» работает весь фильтр целиком. Отдельный переключатель
      ...
      {/* На «Капитале» живой только период: остаток на дату складывается из
          всей истории до неё, и фильтры по счетам, категориям и суммам его не
          меняют. Раз не меняют — гасим, иначе на экране два фильтра «Счета»
        ...
        <div className="flex items-center gap-2 flex-wrap mb-3">
          ...
          <div className="text-sm flex items-center gap-1.5 mr-1">
            ...
            {/* «11 из 29» — части одного числа, поэтому одним стилем.
                «из N» появляется, только когда фильтр часть счетов прячет. */}
            <span className="tabular-nums">
              {visibleRows.length}
          ...
          {/* Итог по видимому списку. Показываем всегда, когда есть что
              складывать: при включённых фильтрах это ответ на «сколько всего
              ...
              title={
                ...
                (hiddenCount > 0
                  ? hiddenCount === dormantCount
                    ...
                    : dormantCount > 0
                      ? `. Не показано ещё ${hiddenCount}: ${dormantCount} ${dormantReason}, остальные скрыты фильтрами`
                      : `. Не показано ещё ${hiddenCount} — скрыты фильтрами`
                  : "")
              }
            >
          ...
          <DropdownMenu
              ...
              reserveBadge
              title="Фильтр по признакам счёта"
            >
              {() => (
                <>
                  <CheckItem
            ...
            >
              ...
              <tbody>
                {listItems.map((item) => {
                  ...
                  // одному и тому же фильтру и сходятся по построению.

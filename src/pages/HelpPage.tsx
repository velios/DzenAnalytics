...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: (
      <>
        ...
        <p className="mt-2 text-muted text-xs">
          Двойной клик по строке (или иконка ✏️ при наведении) — открывает модалку
          редактирования. См. раздел «Локальные правки операций» ниже.
        </p>
      </>
    ),
  },
    ...
    body: (
      <>
        <p>
          ...
          шестерёнки в правой части шапки. Сгруппированы по блокам:
        </p>
        <ul className="list-disc list-inside space-y-1 mt-2">
          <li>
            <strong>Источник данных</strong> — переключение между офлайн-CSV и
            онлайн-синхронизацией через Zenmoney API. См. раздел «Источник данных:
            CSV или API» ниже.
          </li>
          <li>
            <strong>Импорт CSV-выгрузки</strong> Дзен-мани (drag-and-drop или клик).
            Два режима: «Заменить» (стереть и загрузить) и «Дополнить» (дедуп по{" "}
            <code>id</code>, добавить только новое).
          </li>
          <li>
            <strong>Валюты и инфляция</strong> — базовая валюта, таблица курсов,
            переключатель поправки на инфляцию (см. отдельные разделы ниже).
          </li>
          <li>
            <strong>Обработка данных</strong> — группировка похожих получателей
            (Магнит / Магнит #1234 / MAGNIT-MOSCOW → один payee).
          </li>
          <li>
            <strong>Резервные копии</strong> — экспорт всей базы в JSON, импорт обратно,
            настройка авто-бэкапа по расписанию. См. раздел «Бэкапы» ниже.
          </li>
          <li>
            <strong>Удалить все данные</strong> — полный сброс IndexedDB с подтверждением.
          </li>
        </ul>
      </>
    ),
    ...
    body: (
      <>
        ...
        <ul className="list-disc list-inside space-y-1 mt-1">
          ...
          <li>
            Изменённые строки помечены иконкой ✏️ в списке. В модалке появляется
            кнопка <strong>«Сбросить правку»</strong> — мгновенный возврат к оригиналу.
          </li>
          <li>
        ...
        <p className="mt-2 text-muted text-xs">
          Что нельзя менять локально: тип операции (доход/расход/перевод). Если
          операция была разнесена в Дзен-мани неправильно — поправьте её там и
          сделайте синк.
        </p>
      </>
    ),
  ...
  {
    id: "concept-cloud-snapshot",
    group: "concepts",
    icon: CloudIcon,
    title: "Облачный снимок (safety net)",
    body: (
      <>
        <p>
          В «Настройки → Облачный снимок» (виден только при подключённом
          токене API) можно сохранить полный «слепок» того, что сейчас в
          вашем Дзен-мани — сырой ответ <code>POST /v8/diff/</code>. Это
          фундамент будущей двусторонней синхронизации: страховка на
          случай, если push-операция из приложения что-то перезатрёт в
          облаке.
        </p>
        <p className="mt-2">
          <strong>Чем отличается от обычного бэкапа:</strong>
        </p>
        <ul className="list-disc list-inside space-y-1 mt-1">
          <li>
            Обычный JSON-бэкап («Резервные копии») — это наш{" "}
            <em>локальный</em> вид данных: транзакции после маппинга,
            overlay-правки, бюджеты, цели и т.п. Помогает восстановить
            локальное состояние браузера.
          </li>
          <li>
            Облачный снимок — это <em>сырой</em> ответ API ровно в той
            форме, в которой его отдал Дзен-мани. Помогает восстановиться
            на стороне облака, если что-то пошло не так.
          </li>
        </ul>
        <p className="mt-2 text-muted text-xs">
          Хранятся последние 5 снимков, старые автоматически вытесняются.
          Каждый можно скачать как JSON-файл (для архива) или удалить.
          Реальное восстановление через push в облако — это Фаза 2 интеграции,
          пока что снимок только хранится и доступен для скачивания.
        </p>
      </>
    ),
  },
  ...
  },
  {
    ...
    body: (
      <>
        <p>
          Когда подключён API, в «Настройках» появляется переключатель{" "}
          <strong>«Авто-синхронизация»</strong>. При включении приложение
          инициирует фоновый <code>diff</code>-запрос примерно раз в 30 минут,
          а также:
        </p>
        <ul className="list-disc list-inside space-y-1 mt-2">
          <li>при первом заходе на сайт после подключения токена;</li>
          <li>
            при возвращении в вкладку (через событие <code>visibilitychange</code>),
            если с последнего синка прошло больше интервала;
          </li>
          <li>
            вручную — кнопка «Синхронизировать сейчас» в настройках.
          </li>
        </ul>
        <p className="mt-2">

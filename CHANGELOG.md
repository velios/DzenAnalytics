...
Новая страница **«50/30/20»**: нужды (обязательные траты), желания (всё остальное) и сбережения — против ориентира 50/30/20, плюс график **нормы сбережений за 12 месяцев** с целевой линией 20%.

Деление нужды/желания берётся из **самого Дзен-мани** — поля «обязательная» у категории. **По умолчанию все категории расходов считаются обязательными** (нужда); желанием становится только то, что вы явно отметили необязательным в блоке ниже.

### ✏️ Редактор «обязательности» расходов в категориях

На странице **«Категории»** появился сворачиваемый блок **«Обязательность расходов в категориях»**: у каждой категории и подкатегории переключатель **обязательная / необязательная** (подкатегории — с отступом под родителем; плюс колонка «Тип категории» — расходная / доходная / расходная-доходная, из API). Изменение сразу видно в 50/30/20 и **синхронизируется в облако** Дзен-мани — тем же безопасным конвейером Push, что и правки операций (выбранный режим отправки, снапшот перед записью).

### 🎨 Единые цвета категорий

Цвет категории теперь одинаковый везде, где она рисуется (диаграммы, графики, Sankey, точки в списках и детализации): сначала берётся из тега Дзен-мани, иначе — стабильный цвет по имени. Раньше Cash-flow, Тренды и Sankey красили категории по позиции, и одна и та же категория выглядела по-разному.

### 🧹 Мелкие правки

- Фильтры счетов/категорий/валют: кнопки **«Выбрать все» / «Снять все»**; по умолчанию всё выбрано показывается отмеченным (раньше галочки были пустыми и это путало).
- «Дубликаты»: окно дней — от 0; единая ширина колонок во всех группах; кнопка «Открыть в редакторе»; пометка **«Не дубликаты»** (исключает группу из проверки) со списком исключений в отдельной модалке.
- «Сравнение»: пресеты периодов переехали в шапку (Месяц · 30 дней · 90 дней · YTD · Свои даты), график расходов показывает **все** категории (раньше — только топ-15) и больше не уезжает в минус по оси.
- «Цели» переименованы в **«Цели & FIRE»**.
- «Топ получателей»: операции без получателя теперь подписаны «Без получателя» и корректно открываются.
- «Удалённые»: кнопка **«Очистить корзину»**.
- Тултипы в таблицах показываются только если текст реально обрезан; убран тултип «двойной клик».
- Drawer: убрана лишняя полоса прокрутки, когда данные помещаются на экран.
- Тёмная тема: читаемые подписи в подсказках Sankey.
- Аннотации: при наведении на вертикальную линию теперь видно название, дату и заметку (раньше подсказки не было).
- Устойчивость: если страница не смогла отрисоваться, показывается понятная заглушка с кнопками «Попробовать снова / Перезагрузить» вместо пустого белого экрана (меню и остальное приложение остаются доступны).

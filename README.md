...
📋 Свежие изменения — см. [CHANGELOG.md](CHANGELOG.md) · актуальная версия **1.3.0**.
...
- **Двусторонняя синхронизация (push)** — отправка локальных правок и удалений
  ...
  суммы зачисления); перенос операции на счёт другой валюты; флип
  мультивалютной операции в перевод. Перед отправкой
  ...
  затереть чужое изменение. **«Зависшие» правки** (сделаны на CSV-импорте, а id
  не совпадает с облачным после перехода на API) можно убрать одной кнопкой.
...
- **Балансы счетов** — список с реальными остатками. Глобальная настройка
  **«Счета вне баланса»** (Настройки → Обработка) включает накопительные/брокерские
  счета (`inBalance:false`) в списки (Дашборд, «Счета») и в «Совокупный баланс»
...
- **Мультивалютные переводы** — в колонке суммы под отправленной ногой
  показывается сумма зачисления в своей валюте в скобках (`↔ 50 000 ₽` → `(550 $)`)
- **Добавление операции** — кнопка **«Добавить»** (в режиме синхронизации с
  Дзен-мани) открывает ту же форму с чистого листа: любой тип
  (расход/доход/возврат/перевод, в т.ч. мультивалютный), дата по умолчанию —
  сегодня, валюта подставляется по счёту. Новая операция сразу видна в списке с
  пометкой **«не синхронизировано»** и уезжает в облако при следующем Push
- **Редактирование операции** — двойной клик по строке (или иконка ✏️):
  - Тип операции (Расход / Доход / Перевод / Возврат); **дата и время**
  - **Категория и подкатегория** — единое поле с двумя колонками: слева категории,
    справа подкатегории выбранной; клик по категории с подкатегориями раскрывает их,
    «Без подкатегории» выбирает саму категорию. Список фильтруется по типу операции
  - Получатель, комментарий, сумма, валюта; **счёт** — с группировкой «Часто
    используемые / Активные / Архивные»
  - Получатель умеет автодополнять из словаря брендов Дзен-мани; имя, которого
    нет в словаре, сохраняется как свободный текст
  - **Переводы** — без категории/подкатегории (при смене типа на «Перевод»
    категория автоматически становится «Перевод»); счёт-отправитель,
    счёт-получатель, «Отправлено» и «Получено». При разных валютах счетов
    «Получено» авто-пересчитывается по курсу синхронизации и редактируемо
...
- **Откат изменений до отправки** — в блоке Push («Настройки → Данные») кнопка
  **«Посмотреть и откатить изменения»** открывает список несинхронизированных правок
  (созданные черновики, изменения, удаления). Любое можно откатить по одному или все
  сразу — локально, не трогая облако
...
- **Корзина «Удалённые»** — кнопка в тулбаре «Операции» открывает страницу
  ...
  ещё и обратно в облако Дзен-мани (страховка от случайного удаления). Кнопка
  **«Очистить корзину»** окончательно убирает удалённые из локального хранилища
  (с подтверждением; в облако ничего не пишется)
...
| Раздел | Что внутри |
|---|---|
| **Cash-flow** (`/cashflow`, в меню «Ещё») | Месячные бары доходов/расходов + линия чистого потока + прогноз 3 сценария (оптимист/реалист/пессимист) + год к году + аннотации + stream graph по категориям + сезонность. **Свой период** (по умолчанию 12 мес) |
| **Категории** (`/categories`) | Donut-чарт (с выносными подписями и drill-in в подкатегории), treemap (с полноэкранным режимом), bar-chart — все три вида показывают **подкатегории**; подсказка с разбивкой по подкатегориям; иерархия; флаги «фиксированная»/«дискретная» 🔒/☕; кликабельные сегменты |
| **Счета** (`/accounts`) | Список счетов в двух видах (**таблица** по умолчанию / карточки) с актуальным балансом счёта (в API-режиме — полной суммой из Дзен-мани, в т.ч. в валюте счёта; в CSV — изменением по фильтрам), изменением за период, поступлениями/списаниями (без переводов) и sparkline; **архивные счета** — отдельной группой ниже активных, кнопка «Скрыть архивные»; stacked area-чарт по топ-8 счетам, area-линия совокупного баланса (со стартовыми остатками, не уходит в минус), drill-down по любому счёту, **калибровка совокупного баланса**. Показываются **все реальные счета** из Дзен-мани, влияющие на баланс (даже без операций за период), а **кредитки и долги отображаются с минусом** (красным) и корректно уменьшают итоговые суммы |
| **Тренды** (`/trends`) | Помесячный таймлайн категорий, bar-chart по дням недели, radar-диаграмма, heatmap «час недели» (24×7), KPI «будни vs выходные». **Свой период** (по умолчанию 12 мес) |
...
| **Потоки** (`/sankey`) | Sankey-диаграмма: источники доходов → бюджет → категории расходов; узел **«Сбережения»** (доход больше трат) или **«Из накоплений»** (трат больше дохода) |
...
- **Регулярные** (`/recurring`) — авто-детект подписок и регулярных трат: одинаковый
  ...
  платежей и общая месячная нагрузка; статус-«светофор» (🟢 активные / 🔴 неактивные —
  молчат дольше ~2 циклов периодичности), неактивные прячутся по умолчанию
- **50/30/20** (`/50-30-20`) — нужды (обязательные траты) / желания / сбережения
  против ориентира 50/30/20 + график нормы сбережений за 12 месяцев с целью 20%.
  Деление берётся из поля **«обязательная»** у категории в Дзен-мани (по умолчанию
  все категории расходов обязательны → нужда; явно необязательные → желание). Менять
  можно в блоке **«Обязательность расходов в категориях»** на странице «Категории»
  (переключатель Обязательная / Необязательная, с синхронизацией в облако)

...
посредников и только на чтение.

> **v0.5** — крупное обновление: онлайн-синхронизация с Zenmoney API,
> страница «Операции» (сквозная лента с lazy-скроллом), локальные правки операций
> поверх данных, авто-калибровка совокупного баланса, иконки и цвета категорий
> из API, бэкапы по расписанию. Подробности — в разделе [«Что нового в v0.5»](#что-нового-в-v05).
...
- **🔗 Онлайн-синхронизация с Zenmoney API.** Альтернатива CSV: введите токен —
  и приложение само подтягивает операции через `POST /v8/diff/`. **Только на
  чтение** — ни одна правка не уходит в облако. Токен хранится локально
  в IndexedDB и нигде не светится.
- **♻️ Инкрементальный синк** с локальным entity-cache: после первой полной
  выгрузки повторные синки тянут только изменения с момента последнего
  `serverTimestamp`. Авто-синк каждые 30 минут можно включить переключателем.
- **🎯 Авто-калибровка совокупного баланса.** Когда подключён API — стартовые
  остатки счетов берутся прямо из ответа `account.balance`. Ручная калибровка
  скрывается, а KPI с первого захода показывает реальные деньги.
- **🎨 Иконки и цвета категорий из API.** Тег-метаданные (`tag.icon`,
  `tag.color`) подтягиваются с серверов Zenmoney; в UI рядом с названием
  категории появляется фирменный цветной значок с эмодзи.
- **📋 Страница «Операции»** (`/transactions`) — сквозная лента всех операций
  под глобальными фильтрами: KPI-сводка, группировка по дням («Сегодня»,
  «Вчера», полная дата), сортировка по дате/сумме, поиск, CSV-экспорт,
  **lazy-скролл** через IntersectionObserver (по 100 строк за раз, без лагов
  на тысячах операций).
- **✏️ Локальные правки операций.** Двойной клик по строке → модалка с
  редактированием категории/подкатегории (комбобокс с автодополнением по
  типу операции), получателя, комментария, суммы, валюты, счёта. Правки —
  **overlay поверх** данных, синхронизация с API их не затирает. Любую
  правку можно сбросить и вернуть оригинал.
- **💾 Бэкапы по расписанию.** Авто-экспорт всей базы (транзакции + бюджеты +
  цели + правила + аннотации + локальные правки) в JSON по таймеру —
  ежедневно / еженедельно / ежемесячно или вручную.
- **🎨 Брендовый UI.** Горизонтальное лого, фирменные иконки шестерёнки и
  знака вопроса в шапке, компактный pill-toggle для светлой/тёмной темы,
  стабильный gutter скроллбара (хедер больше не «прыгает» между страницами).
- **📌 Переработка меню.** «Операции» — в основном меню рядом с «Главной».
  Cash-flow съезжает в «Ещё» как одна из аналитических страниц.
...
- **Редактирование операции** — двойной клик по строке (или иконка ✏️):
  - Категория + подкатегория с автодополнением (фильтр по типу операции)
  - Получатель, комментарий, сумма, валюта, счёт, дата
  - Правки сохраняются локально как overlay поверх данных; синхронизация
    с API их не затирает
...
1. Откройте `https://api.zenmoney.ru/access_token/` (или используйте OAuth-флоу
   из своего скрипта)
2. Скопируйте полученный personal access token
...
- **Read-only гарантия**: тело запроса не содержит секции `transaction`,
  `account`, `tag` — приложение не отправляет в облако никаких изменений,
  даже если вы редактируете операции локально
- **Локальный entity-cache**: сущности (счета, теги, валюты, операции)
  хранятся в IndexedDB; повторный синк отправляет `lastServerTimestamp`
  и тянет только дельту
- **Авто-калибровка**: при каждом синке `account.balance` агрегируется
  в текущий совокупный баланс — ручную калибровку проставлять не нужно
- **Иконки и цвета** категорий приходят из `tag.icon` / `tag.color`
  (ARGB-int → CSS rgba) и применяются в UI на всех страницах
- **Авто-синк** каждые 30 минут — переключатель в настройках
...
На странице **Настройки** есть блок «Резервные копии» с двумя сценариями:
...
│   │   ├── TransactionsPage.tsx       # /transactions  сквозная лента + lazy-скролл (v0.5)
...
│   │   └── ImportPage.tsx             # /import        настройки (API/CSV/валюты/бэкапы/правила)
│   │
...
│   │   ├── EditTransactionModal.tsx   # редактирование операции (overlay-патч) — v0.5
│   │   ├── CategoryDot.tsx            # цветной значок-эмодзи категории из API — v0.5
│   │   ├── Combobox.tsx               # автодополнение с bounded-dropdown — v0.5
...
│   │
│   ├── store/                         # ~16 Zustand-сторов (все персистятся в IndexedDB)
...
│   │   ├── useZenmoneyStore.ts        # токен/sync state Zenmoney API — v0.5
│   │   ├── useCategoryMetaStore.ts    # icon/color категорий из API — v0.5
│   │   ├── useEditsStore.ts           # локальные правки операций (overlay) — v0.5
│   │   └── useBackupStore.ts          # настройки авто-бэкапа — v0.5
│   │
│   ├── lib/zenmoneyApi.ts             # клиент /v8/diff/ + entity-cache — v0.5
│   ├── lib/zenmoneyMap.ts             # маппинг Zenmoney diff → Transaction[] — v0.5
│   ├── lib/zenIconEmoji.ts            # mapping ~165 Zen icon IDs → emoji — v0.5
│   │
...
- **Zenmoney API** — единственный исходящий трафик: `POST api.zenmoney.ru/v8/diff/`
  напрямую из браузера, **без промежуточных серверов**. Тело запроса **никогда**
  не содержит секций мутации (`transaction`, `account`, `tag`) — только
  `currentClientTimestamp`, `serverTimestamp` и list фильтров. По сути — read-only API

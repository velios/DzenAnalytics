...
- **Двусторонняя синхронизация (push)** — отправка локальных правок и удалений
  ...
  комментарий, сумма, валюта, категория, подкатегория и смена типа между
  Расход / Доход / Возврат, а также смена счёта в пределах одной валюты.
  Смена типа на/с «Перевод», перенос на счёт в другой валюте и
  мультивалютные операции пока не передаются. Перед отправкой проверяются
  конфликты: если операцию изменили в облаке после вашей синхронизации,
  правка не отправляется (останется в очереди) — чтобы не затереть чужое
  изменение.

...
- **Двусторонняя синхронизация (push)** — отправка локальных правок и удалений
  ...
  Расход / Доход / Возврат и на/с «Перевод», а также смена счёта (в т.ч.
  счетов перевода) в пределах одной валюты. Перенос на счёт в другой валюте,
  мультивалютные переводы (между счетами разной валюты) и FX-операции пока
  не передаются. Перед отправкой проверяются конфликты: если операцию
  изменили в облаке после вашей синхронизации, правка не отправляется
  (останется в очереди) — чтобы не затереть чужое изменение.
